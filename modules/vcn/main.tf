...
# NAT Gateway
#---------------------
resource "oci_core_nat_gateway" "main_nat_gw" {
  compartment_id = var.compartment_ocid
...
resource "oci_core_route_table" "private_rt" {
  ...
  route_rules {
    destination       = "0.0.0.0/0"
    destination_type  = "CIDR_BLOCK"
    network_entity_id = oci_core_nat_gateway.main_nat_gw.id
  }
...
resource "oci_core_security_list" "public_sl" {
  compartment_id = var.compartment_ocid
  vcn_id         = oci_core_vcn.main_vcn.id
  display_name   = "${var.public_subnet_display_name}_sl"
  ...
  ingress_security_rules {
    protocol    = "all"
    source      = "10.0.0.0/18" # Replace with remote CIDR if needed
    source_type = "CIDR_BLOCK"
    description = "Allow traffic from external network range"
  }
  ...
  freeform_tags = merge(
    {
      Name     = "${var.public_subnet_display_name}_sl"
...
resource "oci_core_security_list" "private_sl" {
  compartment_id = var.compartment_ocid
  vcn_id         = oci_core_vcn.main_vcn.id
  display_name   = "${var.private_subnet_display_name}_sl"
  ...
  freeform_tags = merge(
    {
      Name     = "${var.private_subnet_display_name}_sl"
